...
import { Button, Card, EmptyState, Header, Pill, Screen, Stat, colors, styles } from "../../components/doctor/ui";
  ...
  return (
    ...
    >
      ...
      <View style={local.grid}>
        <QuickLink icon="checkbox" label="Attendance" tone="success" to="/(doctor)/attendance" />
        <QuickLink icon="notifications" label="Notify" tone="warning" to="/(doctor)/notifications" />
        <QuickLink icon="apps" label="Classes" tone="info" to="/(doctor)/classes" />
        <QuickLink icon="library" label="Subjects" tone="brand" to="/(doctor)/subjects" />
        <QuickLink icon="calendar" label="Weeks" tone="slate" to="/(doctor)/weeks" />
        <QuickLink icon="git-network" label="Hierarchy" tone="info" to="/(doctor)/hierarchy" />
        <QuickLink icon="search" label="Students" tone="brand" to="/(doctor)/student-search" />
        <QuickLink icon="bar-chart" label="Analytics" tone="success" to="/(doctor)/analytics" />
        <QuickLink icon="mail" label="Messages" tone="warning" to="/(doctor)/messages" />
      </View>

      <Header
        title="Active lectures"
        subtitle={`${recording.length} currently recording`}
        action={<Button title="Live" onPress={() => router.push("/(doctor)/live")} variant="secondary" />}
      />
      {recording.length ? (
