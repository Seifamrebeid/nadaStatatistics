/**
 * Doctor weeks — CRUD on weeks/{id}. Mirrors web-doctor DoctorWeeks.
 * Limited to classes belonging to the doctor's subjects.
 */
...
export default function DoctorWeeks() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [subjSnap, clsSnap, weekSnap, lecSnap] = await Promise.all([
      ...
      const subjIds = new Set(subjSnap.docs.map((d) => d.id));
      const myClasses = clsSnap.docs.map((d) => ({ id: d.id, ...d.data() })).filter((c) => subjIds.has(c.subject_id));
      const clsIds = new Set(myClasses.map((c) => c.id));
      const myWeeks = weekSnap.docs.map((d) => ({ id: d.id, ...d.data() })).filter((w) => clsIds.has(w.class_id));
      myWeeks.sort((a, b) => {
        const ca = (myClasses.find((c) => c.id === a.class_id)?.name || "");
        const cb = (myClasses.find((c) => c.id === b.class_id)?.name || "");
  ...
  const displayedRows = useMemo(
    () => filterClass ? rows.filter((r) => r.class_id === filterClass) : rows,
    [rows, filterClass],
  );
  ...
  function openEdit(r) {
    ...
    setForm({
      class_id: r.class_id || "",
      week_number: r.week_number != null ? String(r.week_number) : "",
      title: r.title || "",
      date: r.date || "",
  ...
  async function save() {
    if (!form.class_id || !form.week_number) {
      Alert.alert("Missing fields", "Class and week number are required.");
      return;
    }
    setSaving(true);
    try {
      ...
      if (editing) {
        await updateDoc(doc(db, "weeks", editing.id), payload);
      } else {
  ...
  async function softDelete(r) {
    Alert.alert("Disable week?", `Week ${r.week_number} will be hidden but not deleted.`, [
      { text: "Cancel", style: "cancel" },
      {
        text: "Disable", style: "destructive",
        onPress: async () => {
          try {
            await updateDoc(doc(db, "weeks", r.id), { active: false });
            load();
  ...
  return (
    <Screen refreshControl={<RefreshControl refreshing={loading} onRefresh={load} />}>
      ...
      {classes.length === 0 ? (
        <Text style={{ color: colors.muted, fontSize: 12 }}>You need a class first. Add one from Classes.</Text>
      ) : null}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, marginTop: 6 }}>
          ...
          {classes.map((c) => (
            <Pressable
              ...
              style={[local.chip, filterClass === c.id && local.chipActive]}
            >
              <Text style={[local.chipText, filterClass === c.id && local.chipTextActive]} numberOfLines={1}>
                {c.name || c.id}
              </Text>
            </Pressable>
          ))}
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
            <View style={{ flex: 1, minWidth: 0 }}>
              ...
              <Text style={{ color: colors.faint, fontSize: 11, marginTop: 4 }}>
                {fmtDate(r.date) || "no date"} • lecture: {r.lecture_id || "—"}
              </Text>
            </View>
      ...
      {!loading && displayedRows.length === 0 ? (
        <EmptyState title="No weeks" body="Add a week with '+ New week' above." />
      ) : null}

      <Modal visible={modalOpen} animationType="slide" transparent={false} onRequestClose={close}>
        <Screen>
          <Header
            title={editing ? "Edit week" : "New week"}
          ...
          <Card>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, marginTop: 6, marginBottom: 8 }}>
              {classes.map((c) => (
                ...
                >
                  <Text style={[local.chipText, form.class_id === c.id && local.chipTextActive]} numberOfLines={1}>
                    {c.name || c.id}
                  </Text>
                </Pressable>
              ))}
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, marginTop: 6, marginBottom: 8 }}>
              ...
              {lectures.map((l) => (
                <Pressable
                  key={l.id}
                  onPress={() => setForm({ ...form, lecture_id: l.id })}
                  style={[local.chip, form.lecture_id === l.id && local.chipActive]}
                >
                  <Text style={[local.chipText, form.lecture_id === l.id && local.chipTextActive]} numberOfLines={1}>
                    {l.title || l.id}
                  </Text>
                </Pressable>
              ))}
