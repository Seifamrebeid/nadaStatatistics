...
export default function AdminLectures() {
  ...
  function openCreate() {
    setForm({ title: "", doctor_id: doctors[0]?.id || "",
              status: "scheduled", enrolled_student_ids: [] });
    setModal("create");
  }
  ...
  async function remove(row) {
    ...
    try { await api.delete(`/api/lectures/${row.id}`); await loadAll(); }
    catch (e) { alert(e.response?.data?.error || e.message); }
  }
  ...
  const columns = [
    ...
    { key: "count",  label: "Enrolled",
      render: (r) => (r.enrolled_student_ids?.length ?? 0) },
  ];

  const actions = (r) => (
    <div className="flex gap-2 justify-end">
      <button onClick={() => regenerateReport(r)} className="text-brand hover:underline">Report</button>
  ...
  return (
    <div>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)}
             title={modal === "create" ? "Create lecture" : `Edit ${modal?.row?.title || ""}`}
             footer={<>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="border rounded p-2 max-h-40 overflow-auto space-y-1">
              ...
              {students.length === 0 && <div className="text-slate-500 text-sm">No students yet — create one on the Students tab.</div>}
            </div>
          </div>
        </div>
      </Modal>
