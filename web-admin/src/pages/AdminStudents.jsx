...
export default function AdminStudents() {
  ...
  async function remove(row) {
    ...
    try { await api.delete(`/api/students/${row.id}`); await load(); }
    catch (e) { alert(e.response?.data?.error || e.message); }
  }
  ...
  const columns = [
    ...
    { key: "enrolled", label: "Face",
      render: (r) => (r.face_enrolled_at ? "✓" : (r.face_encoding ? "✓" : "—")) },
  ];
      ...
      <Modal open={modal === "create"} onClose={() => setModal(null)}
        ...
        {savedTempPw && (
          <div className="mt-3 p-3 bg-emerald-50 border border-emerald-300 text-sm rounded">
            Temporary password: <code className="font-mono">{savedTempPw}</code> — share with the new student.
          </div>
        )}
      </Modal>
