/**
 * Admin classes — full CRUD. Admin can pick subject from all subjects.
 */
...
const BLANK = {
  subject_id: "", name: "", section: "",
  academic_year: String(new Date().getFullYear()),
  term: "", enrolled_student_ids: [], active: true,
};
...
export default function AdminClasses() {
  ...
  const subjMap = useMemo(() => Object.fromEntries(subjects.map((s) => [s.id, s.name || s.id])), [subjects]);
  const displayed = useMemo(() =>
    filterSubject ? rows.filter((r) => r.subject_id === filterSubject) : rows,
    [rows, filterSubject]);

  function openCreate() { setEditing(null); setForm({ ...BLANK, subject_id: subjects[0]?.id || "" }); setStudentFilter(""); setModalOpen(true); }
  function openEdit(r) {
    setEditing(r);
    setForm({
      subject_id: r.subject_id || "", name: r.name || "", section: r.section || "",
      academic_year: r.academic_year || "", term: r.term || "",
      enrolled_student_ids: r.enrolled_student_ids || [], active: r.active !== false,
    });
    setStudentFilter(""); setModalOpen(true);
  }
  function close() { setModalOpen(false); setEditing(null); setForm(BLANK); }

  function toggleStudent(id) {
    setForm((p) => {
      const set = new Set(p.enrolled_student_ids);
      set.has(id) ? set.delete(id) : set.add(id);
      return { ...p, enrolled_student_ids: [...set] };
    });
  }

  async function save() {
    if (!form.subject_id || !form.name.trim()) { Alert.alert("Missing", "Subject and class name are required."); return; }
    setSaving(true);
    try {
      const payload = {
        subject_id: form.subject_id, name: form.name.trim(),
        section: form.section.trim(), academic_year: form.academic_year.trim(),
        term: form.term.trim(), enrolled_student_ids: form.enrolled_student_ids,
        active: !!form.active,
      };
      if (editing) await updateDoc(doc(db, "classes", editing.id), payload);
      else await addDoc(collection(db, "classes"), { ...payload, active: true, created_by: user?.uid || null, created_at: serverTimestamp() });
      close(); load();
  ...
  async function softDelete(r) {
    Alert.alert("Disable class?", `${r.name} will be hidden.`, [
      { text: "Cancel", style: "cancel" },
      { text: "Disable", style: "destructive", onPress: async () => {
        try { await updateDoc(doc(db, "classes", r.id), { active: false }); load(); }
  ...
  return (
    <Screen refreshControl={<RefreshControl refreshing={loading} onRefresh={load} />}>
      <Header title="Classes" subtitle="All classes across subjects" action={<Button title="Back" onPress={() => router.back()} variant="ghost" />} />
      ...
      <Card>
        <Text style={{ color: colors.muted, fontSize: 11, fontWeight: "700", letterSpacing: 0.5 }}>FILTER BY SUBJECT</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, marginTop: 6 }}>
          <Pressable onPress={() => setFilterSubject("")} style={[local.chip, !filterSubject && local.chipActive]}>
            <Text style={[local.chipText, !filterSubject && local.chipTextActive]}>All</Text>
          </Pressable>
          {subjects.map((s) => (
            <Pressable key={s.id} onPress={() => setFilterSubject(s.id)} style={[local.chip, filterSubject === s.id && local.chipActive]}>
              <Text style={[local.chipText, filterSubject === s.id && local.chipTextActive]} numberOfLines={1}>{s.name || s.id}</Text>
            </Pressable>
          ))}
        </ScrollView>
      </Card>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
            <View style={{ flex: 1, minWidth: 0 }}>
              ...
              <Text style={{ color: colors.muted, fontSize: 12, marginTop: 2 }}>
                {subjMap[r.subject_id] || r.subject_id} • Section {r.section || "—"}
              </Text>
      ...
      {!loading && displayed.length === 0 ? <EmptyState title="No classes" body="Add one with '+ New class' above." /> : null}

      <Modal visible={modalOpen} animationType="slide" onRequestClose={close}>
        <Screen>
          <Header title={editing ? "Edit class" : "New class"} action={<Button title="Cancel" onPress={close} variant="ghost" />} />
          <Card>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, marginTop: 6, marginBottom: 8 }}>
              {subjects.map((s) => (
                <Pressable key={s.id} onPress={() => setForm({ ...form, subject_id: s.id })}
                  style={[local.chip, form.subject_id === s.id && local.chipActive]}>
                  <Text style={[local.chipText, form.subject_id === s.id && local.chipTextActive]} numberOfLines={1}>{s.name || s.id}</Text>
                </Pressable>
              ))}
            </ScrollView>
