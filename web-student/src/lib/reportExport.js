...
// Mirror of web-admin/src/lib/reportExport.js (kept duplicated so each
// portal stays a self-contained Vite project).
...
export function exportPdf({ filename, title, subtitle, rows, columns, sheets }) {
  ...
  for (let i = 0; i < tables.length; i++) {
    ...
    if (title || t.name) {
      ...
      doc.text(t.name || title || "Report", 40, y); y += 18;
    }
    if (subtitle && i === 0) {
      doc.setFontSize(10); doc.setTextColor("#64748b");
      doc.text(subtitle, 40, y); y += 14;
    }
    doc.setFontSize(8); doc.setTextColor("#94a3b8");
    doc.text(`Generated ${new Date().toLocaleString()}`, 40, y); y += 14;
