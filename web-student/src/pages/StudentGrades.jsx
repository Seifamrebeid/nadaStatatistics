  ...
  async function load() {
    ...
    try {
      const snap = await getDocs(
        query(collection(db, "grades"), where("student_id", "==", profile.linked_id)),
      );
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center text-slate-500">No grades found for your account.</td>
                </tr>
              )}
            </tbody>
          </table>
